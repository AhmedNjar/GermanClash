...
import com.app.germanclash.presentation.feature.game.GameScreen
import com.app.germanclash.presentation.feature.modeselect.ModeSelectScreen
import com.app.germanclash.presentation.feature.results.ResultsScreen
import com.app.germanclash.presentation.feature.room.RoomScreen
    ...
    NavHost(
        ...
        composable(route = Screen.ModeSelect.route) {
            ModeSelectScreen(
                ...
                onPlaySolo = {
                    val soloRoomId = "SOLO_${UUID.randomUUID()}"
                    navController.navigate(Screen.GameGraph.createRoute(soloRoomId))
                },
                onHostGame = {
                    val hostRoomId = "$HOST_ROOM_PREFIX${UUID.randomUUID()}"
                    navController.navigate(Screen.RoomGraph.createRoute(hostRoomId))
                },
                onJoinGame = {
                    val joinRoomId = "$JOIN_ROOM_PREFIX${UUID.randomUUID()}"
        ...
        composable(
            ...
            ResultsScreen(
                ...
                onPlayAgain = {
                    ...
                    val nextRoomId = when {
                        finishedRoomId.startsWith(HOST_ROOM_PREFIX) -> "$HOST_ROOM_PREFIX${UUID.randomUUID()}"
                        finishedRoomId.startsWith(JOIN_ROOM_PREFIX) -> "$JOIN_ROOM_PREFIX${UUID.randomUUID()}"
                        else -> null
                    }
                    if (nextRoomId != null) {
